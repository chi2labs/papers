...
The *Chi Square Mobile App* was orignally developed ot augment the feedback loop in a traditional classrooms settings. Given the emerging context we decided to make the necessary changes to repurpose the development for use in the situation we found ourselves in. The app was built with several R packages, the most important ones of which were: *shinyMobile*[@shinyMobile:2020] and *chi2Mobile*[@chi2Mobile:2020]; and *Android Studio*[@androidstudio:2020], and the necessary updates adjustments were made before it was made available in the Google play store. The system has two main interfaces, one for students and one for instructors.

## Student App
The student app prompts the student when a request for feedback has been made, typically immediately following the online session. The template used for this field test included star-ratings on four questions, and two open-ended question where the students could enter concepts learned and request reviews of concepts that were not clearly understood. The app also allows for access to a student facing analytics dashboard, and a calendar interface. 

In Figure \ref{fig:studentapp} we see the main interfaces for the student.
...
The instructor app provides functionality for the instructor to request feedback (or automate process this with an scheduler) using one of the templates available, or by creating a custom questionaire. The instructor has access to summary statistics for the feedback given, including linguistic analyses for the open-ended question types. There is also a recommendation engine which will propose topics for review and make content recommendations. The instructor app also provides analytics on the progress of students, and ties in to an early warning systems to detect students at high risk of failure. 
...
As pointed out by e.g. @dietrichson_2020blog distribution of grades do typically not conform to statistical normality. In the case of the data at our disposal, the grades are ostensibly given on a scale from zero to ten, with four being the lowest passing grade. In practice the lower end of the scale is not used and a two (2) is often granted as a universal *Fail*, and zero (0) suggests withdrawal from the course on the part of the student. For our statistical modeling we assumed that the variable *course grade* would be measured on an ordinal scale, with the range (0,4] encoded as *Fail*. 
...
While there is an overall strong and significant correlation between the variables in question and the outcome variable, we wanted explore these relationships in more detail. We therefore fitted an ordinal regression model [@MASS2021; @williams:2020] with the raw number of concepts learned and review requests as independent variables. This approach allows us to estimate an effect size for the coefficient as well as the different intercepts. We are also able to deal with potential interaction effects between the independent variables. The results of the regression analysis are reported in Table \@ref(tab:ordinal-regression).
...
We see that there is a negligible interaction effect, furthermore it is worth noting that the effect size for *review requests* is significantly higher than that for *concepts learned*. As for the intercept estimates, we see that they increase as the intercept thresholds increase. This might indicate that a working feedback loop, like the one our app aspires to provide, benefit the stronger students more than the weaker ones. 

it is clear that the 
Statistical significance was found for both *concepts learned* and *review requests*, but it is worth noticing that the effect size for review requests is almost twice that of reported learning. It is also worth noting that the coefficients are significantly lower for the weaker students than the stronger ones. This results seems to indicate that having a working feedback loop is more beneficial to mid to stronger students than the weaker ones. 
