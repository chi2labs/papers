...
```{r}
x <- .extract_top_ngrams_by_week_of_course(unigrams)
.create_ngram_matrix <- function(x, n_col = 7, header_prefix = "Week ") {
   x %>% 
     group_by(week_of_course) %>% 
     tally %>% pull(n) %>% max -> n_rows_per_week
  my_vec <- sapply(1:max(x$week_of_course), function(idx){
    x %>% 
      filter(week_of_course == idx) %>% 
      pull(ngram) %>% 
      as.character()-> tmp
    # Ensure it has n_rows_per_week
    c(paste0(header_prefix, idx), tmp)
  },simplify = TRUE, USE.NAMES = FALSE) 
  # # # Calculate number of rows:
  x %>%
    group_by(week_of_course) %>%
    tally %>% pull(n) %>% max -> n_rows_per_week
  n_row <- ceiling(max(x$week_of_course)/ n_col)*n_rows_per_week+
    ceiling(max(x$week_of_course)/n_col)
  ret <- matrix(nrow = n_row,ncol=n_col)
  # # 
  week_iter <- 1
  for(i in seq(1,n_row, by = 11)){
   ret[i:(n_rows_per_week+i),1:n_col] <-  
     my_vec[1:(n_rows_per_week+1), week_iter:(week_iter+n_col-1) ]
   week_iter <- week_iter + n_col
 }
  ret
} 

.create_ngram_matrix(x) -> mytest
```


 
 Table \@ref(tab:unigram-table-concepts-learned) and table \@ref(tab:unigram-table-review-requests) 
